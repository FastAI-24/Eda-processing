...
| X_test 형상        | **(9,060, 51)** ※취소 거래 212건 필터링          |
...
평가 시스템은 다음 포맷을 요구합니다:

| 컬럼     | dtype            | 설명             |
| -------- | ---------------- | ---------------- |
| `target` | **int64 (정수)** | 예측 가격 (만원) |

> **주의사항**:
>
> - `target` 컬럼은 반드시 **정수형(integer)**이어야 합니다 (float 불가)
> - 원본 test 데이터 전체(9,272건)에 대한 예측이 필요합니다
> - 전처리 시 취소 거래(212건)가 필터링되므로, 제출 시 원본 행 수로 복원 필요

### 제출 파일 생성 흐름

```mermaid
graph LR
    A[LightGBM 예측 9060건] --> B[원본 test 9272건 매핑]
    B --> C[취소 거래 212건은 중간값 대체]
    C --> D[float → int 반올림]
    D --> E[submission.csv 저장]
```
