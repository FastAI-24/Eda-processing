...
├── src/
│   ├── preprocessing/            # 전처리 파이프라인 (Strategy Pattern)
│   │   ├── base.py               # PreprocessingStep, PreprocessingContext
│   │   ├── config.py             # PreprocessingConfig (설정 중앙 관리)
│   │   ├── steps.py              # 21개 전처리 Step 구현
│   │   ├── pipeline.py           # PreprocessingPipeline (Chain of Responsibility)
│   │   ├── run_preprocessing.py  # 전처리 실행 엔트리포인트
│   │   ├── visualizer.py         # EDA 시각화
│   │   └── submission.py         # Baseline submission 생성
│   ├── modeling/                  # 모델링 파이프라인
│   │   ├── base.py               # BaseModel, TrainingResult
│   │   ├── config.py             # ModelConfig (하이퍼파라미터 중앙 관리)
│   │   ├── models.py             # LightGBM / XGBoost / CatBoost 구현
│   │   ├── trainer.py            # Trainer (K-Fold CV, Fold 내 TE)
│   │   ├── ensemble.py           # EnsembleTrainer (가중 평균 앙상블)
│   │   ├── tuner.py              # HyperparameterTuner (Optuna)
│   │   └── run_training.py       # 학습 실행 엔트리포인트
│   ├── features/                  # 피처 엔지니어링 유틸리티
│   ├── data/                      # 데이터 로더
│   └── submission/                # 제출 파일 생성
├── scripts/
│   ├── setup_remote_env.sh        # 원격 GPU 서버 환경 구축
│   └── run_remote.sh              # 코드 업로드 → 실행 → 결과 다운로드
├── notebooks/                     # EDA + 전처리 노트북
├── assets/data/                   # 원본 데이터 (git 미추적)
├── outputs/                       # 모델 출력, submission (git 미추적)
└── summary_exp02~08.md            # 실험 이력 문서
```
...
```bash
# 3-모델 앙상블 (LightGBM + XGBoost + CatBoost)
uv run python -m src.modeling.run_training --ensemble

# 단일 모델 학습
uv run python -m src.modeling.run_training --train-only

# Optuna 하이퍼파라미터 튜닝
uv run python -m src.modeling.run_training --tune
```
...
# 앙상블만 실행
bash scripts/run_remote.sh --ensemble
```
...
|------------------|-------------------|
| Target이 right-skewed (4자릿수 범위) | **log1p 변환** (Step 9) |
| 결측 80% 이상 컬럼 다수 | **80% 임계값**으로 제거, k-* 피처(60~70%)는 보존 (Step 2) |
| 좌표 결측 ~78%이지만 위치↔가격 상관 높음 | **Kakao API + 시군구 평균 보간** (Step 6) |
| 아파트명 고유값 20,000+ | One-Hot 불가 → **Fold 내 Target Encoding** (trainer.py) |
| 2020년 이후 급격한 가격 상승 | **시간 기반 Sample Weight** + **2017+ 필터링** (Step 3.8) |
| 전용면적·층이 target과 상관 높음 | **교호작용 피처** 생성 (Step 7.7) |
| 강남·서초·송파 고가 집중 | **좌표 K-Means 클러스터링** → 생활권 평균가격 (Step 6.8) |
| 교통 편의성과 가격 상관 | **BallTree 기반 교통 피처** (Step 6.7) |
| 과거 데이터의 유령 지하철 문제 | **2017년 이후 데이터만 학습** (Step 3.8) |
...
|----------|----------|------|
| 0~10% | 전용면적, 층, 건축년도 | **보존** |
| 60~70% | k-관리방식, k-난방방식, k-복도유형 | **보존** |
| **80~99%** | 해제사유발생일, 등기신청일자 등 | **제거** |

> **예외**: 좌표X, 좌표Y는 결측이 높아도 **절대 제거 안 함** (거리 피처 계산 필수)

### Step 2.5. Float→Int64 타입 변환

- `본번`, `부번`, `k-전체동수`, `k-전체세대수`, `주차대수` 등 정수형 데이터를 가진 Float 컬럼을 `Int64`(Nullable Integer)로 변환

### Step 3. 컬럼명 정리 (LightGBM 호환)

- LightGBM이 `{}[]"\\,()=` 특수문자에서 파싱 에러를 일으키므로 `_`로 치환

### Step 3.5. 날짜/주소 파생 피처

| 피처 | 산식 | 근거 |
|------|------|------|
| `계약일자` | `계약년월 × 100 + 계약일` | 시계열 정렬 |
| `구` | 시군구 → 2번째 토큰 | 구 단위 위치 정보 |
| `동` | 시군구 → 3번째 토큰 | 동 단위 위치 정보 |
| `건물나이` | `계약연도 − 건축년도` | 감가상각 + 재건축 기대감 |

### Step 3.7. 시간 파생 피처

| 피처 | 산식 | 근거 |
|------|------|------|
| `계약년` | `계약년월 // 100` | 연도별 시장 트렌드 |
| `계약월` | `계약년월 % 100` | 월별 계절성 |
| `계약분기` | `(계약월 - 1) // 3 + 1` | 분기별 패턴 |
| `계약반기` | `(계약월 - 1) // 6 + 1` | 상/하반기 구분 |
| `계약월_sin` | `sin(2π × 계약월 / 12)` | 12월→1월 순환 연속성 |
| `계약월_cos` | `cos(2π × 계약월 / 12)` | 12월→1월 순환 연속성 |

### Step 3.8. 최신 데이터 필터링 (Exp06)

**2017년 이후** 학습 데이터만 사용하여 인플레이션 소음과 '유령 지하철' 문제를 해결합니다.

| 항목 | 값 |
|------|-----|
| 필터링 기준 | `계약년 >= 2017` |
| 학습 데이터 | 약 60% 감소 (2007~2016 제거) |
| 테스트 데이터 | **변경 없음** (전체 유지) |
| 설정 | `config.recent_data_year_from = 2017` (None으로 비활성화 가능) |

> **왜 2017+?** 2017년 이전 거래에는 당시 존재하지 않았던 지하철역 거리가 피처로 반영되는 '유령 지하철' 누수 문제가 있음. 또한 부동산 정책 변화와 대규모 인플레이션으로 과거 가격 패턴이 현재와 구조적으로 다름.
...
> "결측이 많다 = 데이터 부실 단지"라는 정보를 피처로 변환

### Step 6. 좌표 보간 — 3단계 폴백

| 순서 | 방법 | 이유 |
|------|------|------|
| 1차 | **Kakao API** 주소→좌표 조회 (병렬) | 가장 정확한 실좌표 |
| 2차 | **시군구별 평균** 좌표 보간 | 같은 동네 아파트끼리 좌표 유사 |
| 3차 | **전체 평균** 좌표 보간 | 시군구 평균도 없는 경우 폴백 |
...
### Step 6.8. 공간 클러스터링 — K-Means (Exp08)
...
| 대상 | 방법 | 설명 |
|------|------|------|
| 수치형 피처 | **IQR × 3.0** | 전용면적, 층, 건축년도, 주차대수, k-전체세대수 |
| Target | **퍼센타일 0.1%~99.9%** | 극단 오류만 제거 (0.2%) |

> 학습 데이터에서 계산한 IQR 범위를 테스트 데이터에도 **동일 적용**
...
| 좌표 결측 | **Kakao API + 시군구 평균** | 행 제거, 전체 평균만 | 3단계 폴백으로 정확도 최대화 |
...
|------|----------|:---:|
| 3.5 | `계약일자`, `구`, `동`, `건물나이` | 4 |
| 3.7 | `계약년`, `계약월`, `계약분기`, `계약반기`, `계약월_sin`, `계약월_cos` | 6 |
| 5 | `missing_count` | 1 |
| 6.5 | `dist_강남역`, `dist_서울시청`, `dist_여의도` | 3 |
| 6.7 | `nearest_subway_dist`, `subway_count_1km`, `subway_lines_1km`, `nearest_bus_dist`, `bus_count_500m`, `is_real_coord` | 6 |
| 6.8 | `coord_cluster` | 1 |
| 7.5 | `parking_per_household` | 1 |
| 7.6 | `unit_area_avg`, `log_전체세대수`, `log_연면적` | 3 |
| 7.7 | `면적x층`, `면적_건물나이비`, `log_전용면적`, `전용면적_sq`, `층_건물나이비`, `동당세대수`, `is_rebuild_candidate`, `층구간`, `면적대` | 9 |
| | **소계** | **34** |

### Fold 내 Target Encoding 피처 (14개)

| 대상 컬럼 | te_ 피처 | freq_ 피처 |
|----------|---------|-----------|
...
> Step 0에서 테스트 데이터는 행을 유지하므로 전체 9,272건에 대해 예측 가능.
