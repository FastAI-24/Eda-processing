...
### 3.3 시계열 트렌드: 2020년 이후 급등

- 2007~2015년: 상대적 안정기
- 2016~2020년: 완만한 상승
- **2020~2023년: 급격한 가격 상승**

### 3.4 주요 수치형 피처와 Target 상관관계
...
- `해제사유발생일`이 존재하는 행(취소 거래)을 **삭제**
- 기존 Step 2에서 컬럼만 삭제하던 것과 달리, **행 자체를 먼저 제거**하여 학습 노이즈 방지
...
| 판단          | 선택                    | 기각한 대안          | 근거                                    |
| ------------- | ----------------------- | -------------------- | --------------------------------------- |
| 취소 거래     | **행 삭제 (Step 0)**    | 컬럼만 제거          | 취소 거래 행이 학습 노이즈 유발 (병합)  |
...
| 항목               | 값                                               |
| ------------------ | ------------------------------------------------ |
| X_train 형상       | **(1,118,822, 47)**                              |
| X_test 형상        | **(9,272, 47)**                                  |
| y_train_log 형상   | **(1,118,822,)**                                 |
| 최종 컬럼 수       | **47개** (52개에서 고결측 제거 + 파생 피처 추가) |
| 잔여 결측 (학습)   | 13,050,577건                                     |
| 잔여 결측 (테스트) | 98,488건                                         |
| Target 범위 (log)  | 약 11 ~ 19                                       |
| 수치형 피처 수     | **20개**                                         |
| 범주형 피처 수     | **18개**                                         |
| 학습/테스트 컬럼   | **완벽 일치**                                    |
