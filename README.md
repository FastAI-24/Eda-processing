...
버스와 지하철 csv가 제공되어 있고, 유클리드거리로 "가장 가까운 버스 정류소/지하철역 거리, 반경 내 정류소/역 수" 등의 피처를 계획했습니다.
하지만 현재 구현에서는 좌표 결측이 ~78%인 상황에서 시군구별 평균으로 보간한 근사 좌표를 사용하고 있어, 보간된 좌표 기반으로 계산한 버스/지하철 거리 피처가 오히려 노이즈를 추가하는 것은 아닌지 우려됩니다.
