...
# =============================================================================
# [Exp12] Gangnam Gravity - Physical Distance Proof
# 목표:
#   1. Exp08(15,642점) 베이스라인으로 복귀하여 변인 통제. (금리, 브랜드 제거)
#   2. '강남역과의 거리(dist_gbd)' 피처 하나만 추가하여 입지의 연속적 가치 증명.
#   3. 지구 곡률을 반영한 Haversine 공식을 사용하여 거리 정밀도 확보.
# =============================================================================
...
# 3. 상세 피처 엔지니어링 (Exp08 + dist_gbd)
def feature_engineering_exp12(train, test):
    print("\n[Exp12] Engineering: Exp08 Features + Gangnam Distance (Haversine)...")
    ...
    # C. [Core] 강남역 거리 (Haversine)
    # 강남역 좌표: 37.496, 127.027
    print("  -> Calculating Haversine Distance to Gangnam Station...")
...
def train_exp12_gangnam():
    ...
    print(f"Filter Recent (2017+): {len(train)} samples.")

    # 1. Feature Engineering
    train, test = feature_engineering_exp12(train, test)
    ...
    print(f"\n>> Overall Exp12 CV RMSE: {total_rmse:,.0f}")
    
    # 4. 결과 저장
    sub_path = os.path.join(submission_dir, 'submission_exp12_gangnam.csv')
