...
# =============================================================================
# [Exp11] Pure Variable Isolation: is_top_brand
# 목표: Exp08(15,642점) 베이스라인에서 '금리'는 제거하고, 
#      Exp10에서 유의미한 순위(8위)를 기록한 'is_top_brand' 하나만 추가하여 증명.
# =============================================================================
...
# 2. 피처 엔지니어링 (Exp08 + Top Brand Only)
def feature_engineering_exp11(train, test):
    print("\n[Exp11] Isolating 'is_top_brand' on Exp08 Baseline...")
    train_len = len(train)
    ...
    # [Exp11 Core Variable] 상위 브랜드 여부 (Single Variable Proof)
    print("  -> Adding ONLY 'is_top_brand' for proof...")
...
def train_exp11_brand():
    ...
    print(f"Recent Data Filter (2017+): {len(train)} samples.")

    # 1. Feature Engineering
    train, test = feature_engineering_exp11(train, test)
    ...
    print(f"\n>> Overall Exp11 CV RMSE: {total_rmse:,.0f}")
    
    # 4. 저장
    sub_path = os.path.join(submission_dir, 'submission_exp11_brand_only.csv')
